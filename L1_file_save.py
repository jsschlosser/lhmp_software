...
def run():
    ...
    # 1. Copy the original file to the new destination ("Save As" workaround)
    shutil.copy(source_path, target_path)
    
    # 2. Open the newly copied file in append mode ('a') to modify it in place
    with nc.Dataset(target_path, 'a') as dataset:
        ...
        # 5. Update and add global attributes
        # You can use the setncattr method or direct assignment
        dataset.setncattr('history', 'Modified on 2026-07-14: Added new_dimension and new_variable.')
        dataset.experiment_version = "v2.1"

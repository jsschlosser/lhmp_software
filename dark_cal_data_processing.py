...
def dark_current(pathto_raw_dark_cal_data=None):
	...
	pathto_raw_dark_cal_data = f'../dark_current_cal_data_{desired_data_date}.nc'
...
def dark_read(pathto_raw_dark_cal_data=None):
	...
	# set path to data file in netCDF format and read in to python dictionary.
	desired_data_date = input("Enter the date of the desired test data in iso format (YYYY-MM-DD):")
	pathto_raw_dark_cal_data =  [f for f in os.listdir(f'../') if f.__contains__(desired_data_date)]
	image_data = []
	sensor_et = []
	Img_Data = []
	for file in pathto_raw_dark_cal_data:

		dark_cal_data = Dataset(file,'r')
		dark_cal_data_dictionary = {}
		for key in dark_cal_data.variables.keys():
			vals = dark_cal_data.variables[key][:]
			dark_cal_data_dictionary[key] = vals#np.where(vals == '--', np.nan, vals)#vals#	

		image_data.append(dark_cal_data_dictionary['Raw_Signal']) # sensor data to be processed and plotted
		sensor_et.append(dark_cal_data_dictionary['Detector_Exposure_Time']) # sensor temperature to be processed and plotted
		Img_Data.append(image_data.reshape(len(sensor_et),-1)) # reformat pixelated sensor data to be visualized against exposure time
